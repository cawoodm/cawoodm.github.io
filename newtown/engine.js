...
g.Keyboard = function(keyCode) {
    ...
    g.onclick && g.onclick(event);
	}
  if (keyCode!=="click") {
		window.addEventListener("keydown", key.downHandler.bind(key), false);
		window.addEventListener("keyup", key.upHandler.bind(key), false);
	} else {
		document.addEventListener("click", key.clickHandler.bind(key), false);
...
g.GameRender = function() {
  ...
  if (typeof g.preGameRender=="function") {
	...
	g.scene.entities.forEach(function(ent) {
		...
		g.ctx.save();
    // Static objects at e.g. 0,0 are drawn at 0,0 on the current viewport by moving them to the camera position
...
// Placeholders
g.init = function(){dp("No game defined in g.init(). Start coding!")}
