...
`;var Lh=Object.defineProperty,jh=Object.getOwnPropertyDescriptor,Ec=(e,t,n,r)=>{for(var i=r>1?void 0:r?jh(t,n):t,s=e.length-1,a;s>=0;s--)(a=e[s])&&(i=(r?a(t,n,i):a(i))||i);return r&&i&&Lh(t,n,i),i};let jt=class extends ve{constructor(){super(...arguments),this.toasts=[],this.nextId=1}connectedCallback(){super.connectedCallback(),jt.instance=this}disconnectedCallback(){super.disconnectedCallback(),jt.instance===this&&(jt.instance=null)}show(e,t){const n={id:this.nextId++,kind:t?.kind??"info",message:e,...t?.title?{title:t.title}:{}};this.toasts=[...this.toasts,n];const r=t?.durationMs??(n.kind==="error"||n.kind==="warning"?7e3:4e3);n.timer=window.setTimeout(()=>this.dismiss(n.id),r)}dismiss(e){const t=this.toasts.find(n=>n.id===e);t?.timer!=null&&window.clearTimeout(t.timer),this.toasts=this.toasts.filter(n=>n.id!==e)}render(){return $`
      ${this.toasts.map(e=>$`
...
`:"")+`${c.field} — ${u?"click to sort, shift-click to add a sort level, ":"not sortable · "}drag to reorder`+(f?"":" · not filterable");return $`
                <th
                  class=${`${_}${y?" sorted":""}${O?" drag-source":""}${M}${u?"":" no-sort"}`}
                  title=${F}
...
`+Ll(r.map(u=>u.name))),i.length>0&&s.push(`${i.length} view${i.length===1?"":"s"}:
`+Ll(i.map(u=>u.name))),!await e.ui.dialogs.confirm(`The gist does not have these local objects:
...
//# sourceMappingURL=index-Cj8uLdjR.js.map
