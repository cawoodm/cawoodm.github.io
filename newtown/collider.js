...
Collider.prototype.update = function(delta) {
  ...
  for (let i=0; i<this.checks.length; i++) {
    let coll = Collider.collides(this.checks[i].tag1, this.checks[i].tag2, this.checks[i].tol)
...
Collider.prototype.check = function(tag1, tag2, callback, tol) {
  this.checks.push({tag1: tag1, tag2: tag2, callback, tol});
}   
